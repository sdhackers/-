    ...
    <li className="bg-[#0f1724]/50 p-3 rounded-lg flex flex-col gap-3">
      ...
      <div className="flex items-center justify-end gap-2 border-t border-white/10 pt-2">
        ...
        <button
          ...
          className={`p-2 rounded-md transition-colors ${
            item.isFavorite ? 'text-yellow-400 bg-yellow-400/10 hover:bg-yellow-400/20' : 'text-slate-400 bg-slate-600 hover:bg-slate-500'
          }`}
...
export const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, history, onToggleFavorite, onDelete }) => {
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <header className="flex justify-between items-center p-4 border-b border-white/10 flex-shrink-0">
          <h2 className="text-xl font-bold">تاریخچه پرامت‌ها</h2>
        ...
        <div className="flex-grow overflow-y-auto p-4">
          {history.length === 0 ? (
            <p className="text-slate-400 text-center py-8">تاریخچه خالی است.</p>
          ) : (
